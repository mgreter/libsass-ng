...
namespace Sass {
  ...
  public:
    ...
    bool mixinHasContent = false; // Removed in dart?
    ...
    bool _exclusiveAtForRule;
    bool _foundForRuleExpression;
  ...
  protected:
    ...
    virtual Expression* readNamespacedExpression(const sass::string& ns, Offset state);
    ...
    SupportsCondition* readImportSupportsQuery();

    // Consumes a function call within a `supports()`
    // function after an `@import` if available.
    SupportsFunction* tryImportSupportsFunction();
    ...
    CallableArguments* readArgumentInvocation(bool mixin = false, bool allowEmptySecondArg = false);
    ...
    bool lookingAtForRuleContinuation();
    ...
    virtual Expression* readParenthesizedExpression();
    ...
    SelectorExpression* readParentExpression();
    ...
    void readMediaInParens(InterpolationBuffer& buffer);

    void RediaOrInterp(InterpolationBuffer& buffer);

    void readMediaLogicSequence(InterpolationBuffer& buffer, sass::string op);

    void readMediaOrInterp(InterpolationBuffer& buffer);
    ...
    SupportsDeclaration* readSupportsDeclarationValue(Expression* name, Offset& start);
