...
namespace Sass {
  ...
  Stylesheet* StylesheetParser::parseRoot()
  {
    ...
    compiler.envstack.back()->module = root;
  ...
  UseRule* StylesheetParser::readUseRule(Offset start)
  {
    ...
    if (startsWithIgnoreCase(url, "sass:", 5)) {
      ...
      BuiltInMod* module(compiler.getModule24(name));
  ...
  //  return readUseRule(start);
    ...
    if (startsWithIgnoreCase(url, "sass:", 5)) {
      ...
      sass::string name(url.substr(5));
      if (BuiltInMod* module = compiler.getModule24(name)) {
        rule->module32(module);
        rule->root47(nullptr);
      }
      else {
  ...
  ColorExpression* StylesheetParser::readColorExpression(StringScannerState state)
  {
    ...
    Color* color = SASS_MEMORY_NEW(Color, pstate,
      ColorSpaces::rgb, red, green, blue, alpha, original, false);
      ...
      if (scanner.peekChar() != $lparen) {
        ...
        if (name_to_color(plain, red, green, blue, alpha)) {
          return SASS_MEMORY_NEW(ColorExpression, identifier->pstate(),
            SASS_MEMORY_NEW(Color, identifier->pstate(), ColorSpaces::rgb,
